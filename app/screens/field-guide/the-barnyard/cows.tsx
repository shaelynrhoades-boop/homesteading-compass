...
const cowTypes = ['Dairy Cows', 'Beef Cows'];

const nextSteps = [
  {
    name: 'Selling Cow Products',
    alert:
      'Future topics:\n• Selling milk\n• Selling meat\n• Local regulations\n• Pricing & packaging\n• Farmers markets\n\nComing soon 🐄',
  },
  {
    name: 'Find a Cow Group',
    alert:
      'Will connect to:\n• The Waystation\n• Local stockyards\n• Trades & advice\n\nComing soon 🌱',
  },
  {
...
export default function Cows() {
  return (
    ...
    >
      <Text style={styles.title}>Cows</Text>
      <View style={{ alignItems: 'flex-end', marginBottom: 10 }}>
        <InfoButton text="Care basics for cattle housing, pasture, and seasonal health. Connects to: Log Book." />
      </View>

      <Text style={styles.intro}>
        Cows are one of the most versatile and beginner-friendly livestock
        on the homestead. Whether raising for milk, meat, or both, proper setup
        and care are key to a healthy herd.
      </Text>

      {/* Cows Basics */}
      <Text style={styles.sectionHeader}>Cows Basics</Text>
      <View style={styles.grid}>
        {cowsSections.map((item) => (
          <Pressable
            key={item}
            style={styles.button}
            onPress={() =>
      ...
      {/* Cow Types */}
      <Text style={styles.sectionHeader}>Cow Types</Text>
      <View style={styles.grid}>
        {cowTypes.map((item) => (
          <Pressable
            key={item}
          ...
          <Pressable
            ...
            onPress={() =>
              router.push({
                ...
                params: { animal: 'Cows', step: step.name },
              })
            }
          >
