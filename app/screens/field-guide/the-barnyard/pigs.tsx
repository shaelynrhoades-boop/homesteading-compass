...
const pigTypes = ['Heritage Pigs', 'Commercial Pigs'];

const nextSteps = [
  {
    name: 'Selling Pig Products',
    alert:
      'Future topics:\n• Selling meat\n• Selling piglets\n• Local regulations\n• Pricing & packaging\n• Farmers markets\n\nComing soon 🐖',
  },
  {
    name: 'Find a Pig Group',
    alert:
      'Will connect to:\n• The Waystation\n• Local stockyards\n• Trades & advice\n\nComing soon 🌱',
  },
  {
...
export default function Pigs() {
  return (
    ...
    >
      <Text style={styles.title}>Pigs</Text>
      <View style={{ alignItems: 'flex-end', marginBottom: 10 }}>
        <InfoButton text="Housing, feeding, and seasonal care for pigs. Connects to: Log Book." />
      </View>

      <Text style={styles.intro}>
        Pigs are one of the most versatile and beginner-friendly livestock
        on the homestead. Whether raising for milk, meat, or both, proper setup
        and care are key to a healthy herd.
      </Text>

      {/* Pigs Basics */}
      <Text style={styles.sectionHeader}>Pigs Basics</Text>
      <View style={styles.grid}>
        {pigsSections.map((item) => (
          <Pressable
            key={item}
            style={styles.button}
            onPress={() =>
      ...
      {/* Pig Types */}
      <Text style={styles.sectionHeader}>Pig Types</Text>
      <View style={styles.grid}>
        {pigTypes.map((item) => (
          <Pressable
            key={item}
          ...
          <Pressable
            ...
            onPress={() =>
              router.push({
                ...
                params: { animal: 'Pigs', step: step.name },
              })
            }
          >
